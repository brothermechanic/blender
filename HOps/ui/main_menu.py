...
class HOPS_MT_MainMenu(bpy.types.Menu):
    ...
    bl_label = f"HOps: {bl_info['version'][1]}.{bl_info['version'][2]}{bl_info['version'][3]}.{bl_info['version'][4]}"
        ...
        if addon_exists("DECALmachine") or hasattr(wm, 'kitops'):
            if len(bpy.context.selected_objects):
                ...
                if active_object.mode == "OBJECT":
                    layout.operator("view3d.insertpopup", text = "ST3 Loader", icon_value=get_icon_id("QGui"))
...
#            layout.menu("machin3.mesh_machine_plug_utils_menu", text="Plug Utils")
    ...
    def draw_only_with_active_object(self, layout, object):
        ...
        if object.hops.is_pending_boolean:
            ...
            if get_preferences().property.menu_array_type == 'ST3':
                layout.operator("hops.super_array", text="Array (ST3)", icon_value=get_icon_id("Display_operators"))
            else:
    ...
    def draw_lattice_menu(self, layout):
        ...
        layout.operator_context = "INVOKE_DEFAULT"
        op = layout.operator('wm.context_modal_mouse', text='X', icon_value=get_icon_id('gettin id'))
        op.data_path_iter = 'selected_editable_objects'
        op.data_path_item = 'data.points_u'
        op.header_text = 'Points U: %.3f'
        op.input_scale = 0.05

        layout.operator_context = "INVOKE_DEFAULT"
        op = layout.operator('wm.context_modal_mouse', text='Y', icon_value=get_icon_id('gettin id'))
        op.data_path_iter = 'selected_editable_objects'
        op.data_path_item = 'data.points_v'
        op.header_text = 'Points V: %.3f'
        op.input_scale = 0.05

        layout.operator_context = "INVOKE_DEFAULT"
        op = layout.operator('wm.context_modal_mouse', text='Z', icon_value=get_icon_id('gettin id'))
        op.data_path_iter = 'selected_editable_objects'
        op.data_path_item = 'data.points_w'
        op.header_text = 'Points W: %.3f'
        op.input_scale = 0.05
        ...
        layout.separator()
        layout.operator("hops.to_box", text="To_Box", icon_value=get_icon_id("Display_boolshapes"))
    ...
    def draw_boolshape_menu(self, layout):
        ...
        if len(bpy.context.selected_objects) == 1:
            if bpy.context.active_object.modifiers[:]:
                ...
                if get_preferences().property.menu_array_type == 'ST3':
                    layout.operator("hops.super_array", text="Array (ST3)", icon_value=get_icon_id("ArrayCircle"))
                else:
                    layout.operator("hops.adjust_array", text="Array", icon="MOD_ARRAY")
                #layout.operator("clean1.objects", text="Demote", icon_value=get_icon_id("Demote")).clearsharps = False
