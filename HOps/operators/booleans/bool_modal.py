...
LMB + Ctrl - Ignore sort and keep bevel modifiers on inset objects if they don't use vertex groups or bevel weight"""
    ...
    def modal(self, context, event):
        ...
        elif event.type in {'WHEELDOWNMOUSE', 'WHEELUPMOUSE'} or (event.type in {'X', 'Z'} and event.value == 'PRESS'):
            if event.type in {'WHEELDOWNMOUSE', 'X'}:
                self.scroll(1)
            elif event.type in {'WHEELUPMOUSE', 'Z'}:
                self.scroll(-1)
