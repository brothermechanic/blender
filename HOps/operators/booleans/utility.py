...
class HOPS_OT_BoolInset(HOPS_BOOL_OPERATOR, bpy.types.Operator):
    ...
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = """Cuts mesh using Inset Boolean
LMB - Boolean object set to wire / boolshape (DEFAULT)
LMB + Shift - Outset
LMB + Ctrl - Keep small bevels and ignore sort
...
class HOPS_OT_BoolKnife(bpy.types.Operator):
    ...
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = """Cuts mesh using Knife Boolean
...
LMB + Shift - Knife Project"""
    ...
    def execute(self, context):
        return operator.knife(context, self.knife_project)
