...
class HOPS_PT_Button(Panel):
    ...
    def draw(self, context):
        ...
        if button.context == 'OPERATORS':
            ...
            column.operator("hops.about", text = f"HOps: {bl_info['version'][1]}.{bl_info['version'][2]}{bl_info['version'][3]}.{bl_info['version'][4]}",  icon_value=get_icon_id("GreyDisplay_dots"))
            #column.label(text=f"{bl_info['description']}")


            object = context.active_object

            if object and object.mode == 'OBJECT' and object.type == 'MESH':
                column.separator()
