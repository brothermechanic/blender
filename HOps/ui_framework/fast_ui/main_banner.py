...
class Fast_UI():
    ...
    def __init__(self, db):
        ...
        self.font_size = int(12 * self.db.scale_factor)
    ...
    def draw_text(self, key_max_width, offset_y, key_dims):
        ...
        for item in self.mod_items:
            ...
            if key == self.active_mod_name:
                ...
                render_text(text=key, position=(prefs_x_offset + self.panel_padding, offset_y + self.panel_padding + prefs_y_offset), size=self.font_size, color=color)
                render_text(text=desc, position=(prefs_x_offset + self.panel_padding + key_max_width, offset_y + self.panel_padding + prefs_y_offset), size=self.font_size, color=color)
...
class Help():
    ...
    def draw(self, context):
        ...
        if self.help_items != []:
            ...
            total_height = (len(self.help_items) * offset_y) + self.padding_TB
            ...
            for item in self.help_items:
                ...
                key, desc = item[0], item[1]
                color = self.db.colors.Hops_UI_secondary_text_color
                render_text(text=key, position=(offset_x, offset_y + self.panel_padding + prefs_y_offset), size=self.font_size, color=color)
                render_text(text=desc, position=(offset_x + key_max_width, offset_y + self.panel_padding + prefs_y_offset), size=self.font_size, color=color)
