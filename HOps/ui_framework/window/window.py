...
class Window_Gizmo():
    ...
    def clamp_ui_location(self):
        ...
        # Left
        if self.bottom_left[0] < self.width + self.offset:
            self.bottom_left[0] = self.width + self.offset

        # Right
        if self.bottom_left[0] + self.win_width + self.width + self.offset > self.window.db.event.screen_width:
            self.bottom_left[0] = self.window.db.event.screen_width - self.win_width - (self.width + self.offset)

        # Top
        if self.bottom_left[1] + self.win_height + self.width + self.offset > self.window.db.event.screen_height:
            self.bottom_left[1] = self.window.db.event.screen_height - self.win_height - (self.width + self.offset)

        # Bottom
        if self.bottom_left[1] - self.width - self.offset < 0:
            self.bottom_left[1] = self.width + self.offset
