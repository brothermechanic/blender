    ...
    def draw_master(self, context):
        ...
        if self.master.should_build_fast_ui():
            ...
            if get_preferences().ui.Hops_modal_fast_ui_loc_options != 1: #Fast Floating
                ...
                win_list.append(f"{self.mod.factor:.3f}")
                win_list.append(f"{mods}")
            else:
                win_list.append("Smooth")
                win_list.append(f"{self.mod.iterations}")
                win_list.append(f"Factor: {self.mod.factor:.3f}")
            ...
            self.master.receive_fast_ui(win_list=win_list, help_list=help_list, image="Demote", mods_list=mods_list, active_mod_name=active_mod)
