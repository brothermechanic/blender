    ...
    if option.active_material:
        if obj:
            if replace:
                ...
                obj.material_slots[0].material = bpy.data.materials[option.active_material]
...
def prepare(context, option, obj):
    # original_active = bpy.data.objects[context.active_object.name]
    ...
    if option.force:
        ...
        bpy.ops.object.material_slot_add()
        obj.material_slots[-1].material = bpy.data.materials[option.active_material]
...
def add_material(context, obj, mat=None, option=None):
    ...
    existing = [mat for mat in bpy.data.materials]

    if not mat and option:
        mat = bpy.data.mat[option.active_material]

    elif not mat:
        bpy.ops.material.new()
        mat = [mat for mat in bpy.data.materials if mat not in existing][0]

        del existing

    if len(obj.material_slots) > 1:
        if obj.material_slots[-1].material != bpy.data.materials[mat.name]:
            bpy.ops.object.material_slot_add()
            obj.material_slots[-1].material = bpy.data.materials[mat.name]
