...
class HOPS_OT_Conv_To_Shape(bpy.types.Operator):
    ...
    bl_description = """LMB - Converts selection to a primitive
LMB+SHIFT - Creates primitive for each selected object
LMB+CTRL - Parent selection to empties

"""
#general
    individual: bpy.props.BoolProperty(
    name="Individual", description="Create box per object", default = False)
...
#selector
            ...
            if context.mode == 'OBJECT':
                for obj in self.selected:
                    ...
                    if self.primitive_type == 'Empty':
                        ...
                        primitive.matrix_world = obj.matrix_world
                        self.center_origin(primitive)
                        self.make_equal(primitive)
                        primitive.scale*=self.scale 
                        apply_scale_matrix(primitive)
    ...
    def pirmitive_add (self, context, object, vector, scale ) :
        ...
        bmesh.ops.transform(bm, matrix = math.get_sca_matrix(scale), verts = bm.verts)
        bmesh.ops.transform(bm, matrix = Matrix.Translation(vector), verts = bm.verts)
        ...
        if self.copy_1st_bvl and primitive not in self.no_bevel_club:
            ...
            if source:
                ...
                if bevels:
                    ...
                    if source_bvl:
                        dest_bvl = primitive_obj.modifiers.new(type = 'BEVEL', name= source_bvl.name)
                        properties = [p.identifier for p in source_bvl.bl_rna.properties if not p.is_readonly]
                        for prop in properties:
                            setattr(dest_bvl, prop, getattr(source_bvl, prop))
    ...
    def make_equal (self, obj):
        if self.equalize:
            dim_max = max(obj.dimensions)
            obj.dimensions= [dim_max,dim_max,dim_max]
...
def matrix_transfrom (matrix, location = None, rotation = None, scale = None):
    ...
    return mat_loc @ mat_sca @ mat_rot 
